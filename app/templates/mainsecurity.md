...
## Next: turn this into your “real registry”

If you want the next step, I can output this in a strict spec format like:
...
…and I can also generate a **permission registry** (the full list of PermissionIDs with descriptions + risk + MFA/approval flags) so your GPT-5.2 can implement it consistently.

Tell me if you want:

1. **Minimal production set** (like 12 roles), or
2. **Full enterprise set** (all roles above), or
3. A **YAML-style spec** for ingestion.

---

# Implementation notes (PentaVision RBAC)

This document maps directly onto the RBAC tables already present in the UserDB.

## Data model

* **roles**: one row per staff role (ex: `SOC Operator - Tier 1`).
* **permissions**: one row per permission string (ex: `Video.Live.View`, `Video.Export.*`).
* **role_permissions**: joins roles to permissions, with an `effect` of:
  * `allow`
  * `deny`
* **user_roles**: assigns roles to users (optionally property-scoped).

## Matching and precedence

* Permission checks support exact and wildcard permissions.
  * Example: checking `Video.Live.View` also considers `Video.Live.*`, `Video.*`, and `*`.
* **Deny overrides allow** when both match.
* If nothing matches as `allow`, the action is denied.

## Seeding roles/permissions

* The idempotent seed script lives at:
  * `deploy/pentavision_rbac_seed.sql`
* For fresh installs, `deploy/pentavision_schema.sql` includes the RBAC columns.
* For existing installs, run:
  * `deploy/update_databases.sh`
  which applies both `pentavision_schema.sql` and `pentavision_rbac_seed.sql`.
