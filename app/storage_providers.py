...
def build_storage_providers(app: Flask) -> List[StorageProvider]:
    ...
    supports multiple named instances per provider type.
    """
    ...
    # Safety fallback: if no enabled modules exist, ensure at least one provider
    # is available (startup bootstrap should normally prevent this).
    base_dir = (
        app.config.get("LOCAL_STORAGE_PATH")
        or app.config.get("RECORDING_BASE_DIR")
        or os.path.join(app.instance_path, "recordings")
    )
    return [LocalFilesystemStorageProvider(str(base_dir))]
